...
namespace OmcSales.API.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ProductForReturnDTO>>> GetManagerProducts(string email)
        {
            try
            {
                var station = await DbContext.FillingStations.FirstOrDefaultAsync(i => i.ManagerEmail == email);

                var productsForUser = DbContext.Products.Where(i => i.UserId == station.UserId);
            ...
            {
                return BadRequest("Sorry and error occurred");
            }
        }
